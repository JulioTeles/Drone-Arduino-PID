...
float kP_Pitch = 0.6; //0.6
float kI_Pitch = 0; //0.00070
float kD_Pitch = 0.0001; //0.00

float kP_Roll = 0.6; //0.6
float kI_Roll = 0; //0.00070
float kD_Roll = 0; //0.00
...
void loop() {
 ...
 /*---X axis angle---*/
 Roll = 0.98 *(Roll + Gyro_angle_x) + 0.02*Acc_angle_x;
 /*---Y axis angle---*/
 Pitch = 0.98 *(Pitch + Gyro_angle_y) + 0.02*Acc_angle_y;
...
if(elapsedTime>=sampleTime){
  ...
  //Right
  if (pwmRightFront < 11) { pwmRightFront = 11;}
  if (pwmRightFront > 255){ pwmRightFront = 255;}
  if (pwmRightBack < 11) { pwmRightBack = 11;}
  if (pwmRightBack > 255){ pwmRightBack = 255;}
  //Left
  if (pwmLeftFront < 11) { pwmLeftFront = 11;}
  if (pwmLeftFront > 255){ pwmLeftFront = 255;}
  if (pwmLeftBack < 11) { pwmLeftBack = 11;}
  ...
  analogWrite(pinoPWMRF, 0.6*pwmRightFront);
  analogWrite(pinoPWMRB, 0.6*pwmRightBack);
